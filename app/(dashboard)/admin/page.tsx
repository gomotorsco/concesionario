...
type EventsMetrics = {
  ...
  topLists: {
    ...
    origins: { key: string; count: number }[];
    types: { key: string; count: number }[];
  };
  funnel: {
    page_view: number;
    entry_modal_open: number;
    whatsapp_click: number;
    whatsapp_click_vehicle: number;
    lead_submit: number;
  };
  series: { date: string; count: number }[];
  topVehicles: { vehicle_id: number | null; vehicle_name: string | null; count: number }[];
...
export default function DashboardPage() {
  ...
  // Eventos internos (nuevo)
  const [eventsRange, setEventsRange] = useState<"7d" | "30d" | "90d">("7d");
  const [eventsMetrics, setEventsMetrics] = useState<EventsMetrics | null>(null);
  ...
  async function loadEvents(range: "7d" | "30d" | "90d") {
    setEventsLoading(true);
    try {
      const r = await fetch(`/api/events/metrics?range=${range}`, { cache: "no-store" });
      const json = (await r.json()) as EventsMetrics;
      setEventsMetrics(json);
    } catch (e) {
      console.error("Error cargando events metrics:", e);
    } finally {
  ...
  if (loading && !metrics) return <p className="text-slate-400">Cargando…</p>;
  if (!metrics) return <p className="text-slate-400">No se pudieron cargar los datos.</p>;
  ...
  const internalCards = useMemo(() => {
    ...
    return {
      ...
      modal_open: t.entry_modal_open || 0,
      lead_submit: t.lead_submit || 0,
      total_events: eventsMetrics?.totalEvents || 0,
      since: eventsMetrics?.since ? eventsMetrics.since.slice(0, 10) : "-",
      range: eventsMetrics?.range || eventsRange,
    };
  }, [eventsMetrics, eventsRange]);
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={eventsRange}
              onChange={(e) => setEventsRange(e.target.value as any)}
              className="rounded-md border border-slate-700 bg-slate-950/60 px-2 py-1 text-xs text-slate-100"
            >
            ...
            <span className="text-[11px] text-slate-500">
              {eventsLoading ? "Actualizando…" : `Desde ${internalCards.since}`}
            </span>
          </div>
        </div>

        <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card title="Page views" value={internalCards.page_views} />
          <Card title="Clicks WhatsApp (total)" value={internalCards.whatsapp_total} />
          <Card title="Clicks WhatsApp (vehículo)" value={internalCards.whatsapp_vehicle} />
          <Card title="Aperturas modal" value={internalCards.modal_open} />
        </section>

        <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card title="Leads enviados (event)" value={internalCards.lead_submit} />
          <Card title="Total eventos" value={internalCards.total_events} />
          <Card title="Rango" value={internalCards.range} />
          <Card title="Desde" value={internalCards.since} />
        </section>

        {/* Funnel */}
        <div className="bg-slate-950/50 border border-slate-800 p-4 rounded-xl">
          <p className="text-xs text-slate-400 mb-3">Funnel (conteos)</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
            <FunnelItem label="Page view" value={eventsMetrics?.funnel?.page_view ?? 0} />
            <FunnelItem label="Modal open" value={eventsMetrics?.funnel?.entry_modal_open ?? 0} />
            <FunnelItem label="WA click" value={eventsMetrics?.funnel?.whatsapp_click ?? 0} />
            <FunnelItem label="WA veh." value={eventsMetrics?.funnel?.whatsapp_click_vehicle ?? 0} />
            <FunnelItem label="Lead submit" value={eventsMetrics?.funnel?.lead_submit ?? 0} />
          </div>
        </div>

        {/* Top vehículos */}
        <div className="bg-slate-950/50 border border-slate-800 p-4 rounded-xl">
          <p className="text-xs text-slate-400 mb-3">Top vehículos por WhatsApp</p>
          {(!eventsMetrics?.topVehicles || eventsMetrics.topVehicles.length === 0) ? (
            ...
            <div className="divide-y divide-slate-800">
              ...
              {eventsMetrics.topVehicles.map((v) => (
                <div key={`${v.vehicle_id ?? "n"}-${v.vehicle_name}`} className="flex justify-between py-2 text-sm">
                  <span className="text-slate-100">{v.vehicle_name || `ID ${v.vehicle_id}`}</span>
                  <span className="text-slate-200">{v.count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Series por día + breakdowns */}
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <MiniBarChart
            title="Eventos por día (serie)"
            items={(eventsMetrics?.series || []).map((s) => ({ key: s.date, count: s.count }))}
            height={140}
          />
          <MiniBarChart
            title="Dispositivos (top)"
            items={eventsMetrics?.topLists?.devices || []}
            height={140}
          />
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <MiniBarChart
            title="UTM Source (top)"
            items={eventsMetrics?.topLists?.sources || []}
            height={140}
          />
          <MiniBarChart
            title="UTM Campaign (top)"
            items={eventsMetrics?.topLists?.campaigns || []}
            height={140}
          />
        </section>
      </section>
                      ...
                      {editingSeguimiento && (
                        ...
                        >
                          ...
                          <textarea
                            ...
                            onChange={(e) => setSeguimientoDraft(e.target.value)}
                            className="w-full rounded-md border border-slate-700 bg-slate-950/80 px-2 py-2 text-xs text-slate-50 min-h-[80px]"
                            placeholder="Escribí aquí el seguimiento de este lead"
                          />
                          <div className="flex items-center gap-2">
                            <button
                              type="submit"
