...
const items = [
  ...
  { label: "Panel vendedores", href: "/admin/vendedores", pro: true },
...
export function Sidebar() {
  return (
    <aside className="fixed left-0 top-0 z-40 hidden h-screen w-72 overflow-y-auto border-r border-white/10 bg-[#030509] p-5 text-white lg:block">
      ...
      <nav className="space-y-1 pb-20">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-semibold text-slate-300 transition hover:bg-blue-600 hover:text-white"
          >
      ...
      <div className="fixed bottom-5 left-5 w-[248px] text-xs leading-5 text-slate-500">
